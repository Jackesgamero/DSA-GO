...
func ParseStringAndUpdateValue(input, updateKey, newValue string) map[string]interface{} {
	...
	if val, exists := resultMap[updateKey]; exists {
		if _, ok := val.(string); ok {
			resultMap[updateKey] = newValue
		} else if innerMap, ok := val.(map[string]string); ok {
			if _, exists := innerMap[updateKey]; exists {
				innerMap[updateKey] = newValue
			}
		}
		return resultMap
	}
	...
	for _, v := range resultMap {
		if innerMap, ok := v.(map[string]string); ok {
			if _, exists := innerMap[updateKey]; exists {
				innerMap[updateKey] = newValue
				return resultMap
			}
